...
#define ESP_WIFI_PASS      "moje_tajne_heslo"
#define ESP_MAXIMUM_RETRY  10

/*Telegram configuration*/
#define TOKEN "7126098051:AAFctboOuKfV70TCoGuMqFH7s6ITDlaxK_A"
...
#define chat_ID1 "@espxcz_bot"
#define chat_ID2 "6823323469"
...
static void https_telegram_getMe_perform(void) {
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG2, "HTTPS Status = %d, content_length = %lld",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
        ESP_LOGW(TAG2, "Výsledek: %s",buffer);
    } else {
        ESP_LOGE(TAG2, "Error perform http request %s", esp_err_to_name(err));
    }

    ESP_LOGW(TAG2, "Close client");
    esp_http_client_close(client);
    ESP_LOGW(TAG, "Clean");
    esp_http_client_cleanup(client);
}
...
static void https_telegram_getMe_native_get(void) {
    ...
    esp_http_client_close(client);
    esp_http_client_cleanup(client);
    ESP_LOGI(TAG2, "Desde perform esp_get_free_heap_size: %lu", esp_get_free_heap_size ());
}
...
static void https_telegram_sendMessage_native_get(void) {
	...
	*/
    ...
    esp_http_client_close(client);
    esp_http_client_cleanup(client);
    ESP_LOGI(TAG, "esp_get_free_heap_size: %ld", esp_get_free_heap_size ());
}
...
static void https_telegram_sendMessage_perform_post(void) {
	...
	*/
    ...
    esp_http_client_close(client);
    esp_http_client_cleanup(client);
    ESP_LOGI(TAG3, "esp_get_free_heap_size: %ld", esp_get_free_heap_size ());
}
